...
IntSingleList *getIntElementToIntSingleList() {
    int a=1,i=1;
    printf("Задання чисел для списку\n");
    IntSingleList *b = createIntSingleList();
    if(b != NULL) {
    do{
    printf("Задайте число(Щоб закінчити введення, введіть 0): ");
    scanf("%d", &a);
...
int removeIntElementsLessThanAverageValue(IntSingleList *list, int averageValue) {
    if(list == NULL) {
 		return 0;
	}
    ...
    Node *element = list->head;
    printf("Видалення числа, яке менше за середнє\n\n");
    while(element != NULL && element->value < averageValue) {
        Node *temp = element->next;
