...
static void deleteNode(intNode *aNode) // видалення елемента дерева
{
	if (NULL != aNode)
	{
...
static intNode *createNodeWithIntValue(int value) // створення елемента дерева
{
...
int deleteValueFromTree(intTree *aTree, int aValue) // видалення елемента з дерева з його можливим заміщенням (або без нього)
{
	int theResult = 1;
	intNode *theCurrent = aTree->root;
...
void printTree(intNode *aRoot) // вивід дерева у консоль
{
	if (NULL == aRoot)
		return;
